...
fn associated_token_address(mut cx: FunctionContext) -> JsResult<JsString> {
    let address = parse_string_with_error(&mut cx, 0, "address is required")?;
    let token = parse_string_with_error(&mut cx, 1, "token address is required")?;
    Ok(cx.string(convert_json(core::associated_token_address(
        address, token,
    ))))
}
...
fn main(mut cx: ModuleContext) -> NeonResult<()> {
    ...
    cx.export_function("associatedTokenAddress", associated_token_address)?;
    cx.export_function(
